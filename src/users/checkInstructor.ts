...
export class CheckInstructor extends OpenAPIRoute {
    static schema = {
        tags: ["Auth"],
        summary: "Check Instructor",
    ...
    async handle(request: Request, env: any, context: any, data: Record<string, any>) {
        try {
            ...
            const results = await db.select().from(users).where(and(eq(users.role, 'INSTRUCTOR'), eq(users.user_id, env.user_id))).all()
            // let date = new Date()
